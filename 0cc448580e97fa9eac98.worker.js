...
function initSearch(uid,indexes,documents){const search=new js_search__WEBPACK_IMPORTED_MODULE_1__["Search"](uid);const originTokenize=search.tokenizer.tokenize;search.tokenizer.tokenize=function(text){return Object(lodash__WEBPACK_IMPORTED_MODULE_0__["flatten"])(Object(lodash__WEBPACK_IMPORTED_MODULE_0__["flatten"])(text.split('/').filter(Boolean).map(v=>v.split(/\s+/))).map(segment=>{const enReg=/^[a-zA-Z0-9$@$!%*?&#^-_. +]+$/;if(enReg.test(segment)){return originTokenize(segment);}return segment.replace(/[\x00-\x7F]/g,"").split('');}));};indexes.forEach(index=>search.addIndex(index));search.addDocuments(documents);return search;}

/***/ })

/******/ });
//# sourceMappingURL=0cc448580e97fa9eac98.worker.js.map